...
public class JwtFilter extends OncePerRequestFilter {
    ...
    private void authenticateUserIfNecessary(HttpServletRequest request, HttpServletResponse response, String token, String username) {
        if (username != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            LOG.debug("JwtFilter.authenticateUserIfNecessary(request, response, token, username) => username: {}", username);
    ...
    private void logUserRoles(UserDetails userDetails) {
        ...
        authorities.forEach(authority -> LOG.debug("JwtFilter.logUserRoles(userDetails) => User has role: {}", authority.getAuthority()));
    }
